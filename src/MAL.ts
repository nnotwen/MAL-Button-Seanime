...
function init() {
    $ui.register((ctx: any) => {
        console.log("[MAL Button] v2.2.0-beta Initializing...");
        ...
        const observer = new MutationObserver(() => {
            const container = document.querySelector('div[data-anime-meta-section-buttons-container="true"]');

            if (container && !document.getElementById("mal-injected-button")) {
                ...
                btn.type = "button";
                // Copying exact classes from the AniList/Trailer buttons for seamless integration
