...
function init() {
    $ui.register((ctx: any) => {
        console.log("[MAL Button] v2.4.5 (Crash Fix Retry) Initializing...");
        ...
        const injectButton = async (navId?: number) => {
            try {
                ...
                let animeId = navId;
                if (!animeId) {
                    // We can't use window.location anymore, but onNavigate passes safe data
                    // If we are here without navId, we might be on initial load.
                    // Let's try to get ID from the state or skip.
                ...
                if (container) {
                    // Diagnostic: What can we do with elements?
                    if (!navId) { // Only log on init to avoid spam
                        console.log("[MAL Button] Container Methods:", Object.keys(container).join(", "));
                    }
                    ...
                    if (existingBtn) {
                        ...
                        try {
                            if (typeof existingBtn.remove === "function") {
                                ...
                                console.log("[MAL Button] element.remove() is missing. Methods:", Object.keys(existingBtn).join(", "));
                            }
                        } catch (e) {
                    ...
                    if (!navId) {
                        console.log("[MAL Button] Created Element Methods:", Object.keys(btn).join(", "));
                    }
                    ...
                    // MAL Icon (Simple SVG)
                    // Try to detect the correct HTML setting method
                    ...
                    } else if (typeof btn.setHtml === "function") {
                        btn.setHtml(`<span class="md:inline-block">...</span>`); // Simplified if name is different
                    } else {
                        console.log("[MAL Button] No HTML setter found on element.");
                    }
                    ...
                    btn.addEventListener("click", async () => {
                        try {
                            ...
                            if (malId) {
                                ...
                                ctx.action.openUrl(url); // Better than window.open in logic-thread
                            } else {
                    ...
                    console.log("[MAL Button] Button injected successfully.");
            ...
            ctx.screen.onNavigate((data: any) => {
                ...
                if (typeof data === "object" && (data.pathname || data.path)) {
                    ...
                    if (path && (path.includes("/entry") || path.includes("/anime"))) {
                        ...
                        console.log("[MAL Button] v2.4.5 Navigation detected, ID:", searchId);
                        if (searchId) {
                            // Inject with ID
